...
n<-N<-as.integer(Sys.getenv("PANEL_ARMA_GARCH_N", "30"))

T<-as.integer(Sys.getenv("PANEL_ARMA_GARCH_T", "30"))

spatial_rho<-as.numeric(Sys.getenv("PANEL_ARMA_GARCH_RHO", "0.2"))

sample_size<-as.integer(Sys.getenv("PANEL_ARMA_GARCH_SAMPLE_SIZE", "1000"))

mc_seed<-Sys.getenv("PANEL_ARMA_GARCH_SEED", "")

if (nzchar(mc_seed)) {
  set.seed(as.integer(mc_seed))
}

if (!is.finite(spatial_rho) || spatial_rho < -1/(N - 1) || spatial_rho > 1) {
  stop("PANEL_ARMA_GARCH_RHO must define a valid equicorrelation")
}

if (N < 2L || T < 2L || sample_size < 1L) {
  stop("PANEL_ARMA_GARCH_N must be at least 2, T at least 2, and sample_size positive")
}
...
Sigma_star<-(1-spatial_rho)*diag(N) + spatial_rho*l_N%*%t(l_N)

Sigma_star_chol<-chol(Sigma_star)
...
while(dim(a)[1]<sample_size){ 
  ...
  U<-matrix(0,nrow=N,ncol=2*T)
  
  eps0<-rep(0,N)
  h0<-omega
  
  for(t in 1:(2*T)){
    h0<- omega*(1-nu-tau) + nu*h0 + tau*eps0^2  
    
    eps0<-sqrt(h0)*as.vector(t(Sigma_star_chol)%*%rnorm(N))
    #eps0<-sqrt(h0)*sqrt(0.6)*rt(N,5)
    #eps0<-sqrt(h0)*runif(N,-sqrt(3),sqrt(3))
